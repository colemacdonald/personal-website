...
import {
  ...
  AnimatedBackgroundElementType
} from './background-elements/AnimatedBackgroundElement';
...
export class RoomBuilder {
  ...
  withAnimatedElement(e: { x: number; y: number; type: AnimatedBackgroundElementType; scale: any }): RoomBuilder {
    this.room.backgroundElements.push(
      new AnimatedBackgroundElement({ x: e.x, y: e.y, inFrontOfPlatforms: true, type: e.type, scale: e.scale })
    );
    return this;
  }
  ...
  static buildRandomRoom(options: GameOptions): Room {
    ...
    let i = 0;
    while (room.monsters.length < options.numberOfMonsters && i < room.platforms.length) {
      if (
    ...
    for (let i = 0; i < 20; i++) {
      room.coins.push(
        new Coin({
          x: Math.random() * (options.width - options.coinMargin * 2) + options.coinMargin,
          y: Math.random() * (options.height - options.coinMargin * 2) + options.coinMargin
        })
      );
    }

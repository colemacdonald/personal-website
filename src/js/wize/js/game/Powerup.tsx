...
export class Powerup {
  ...
  public static Health(x: number, y: number): Powerup {
    return new Powerup({
      ...
      instructions: '',
      pauseDuration: 0,
      frames: PowerupFrames[PowerupSpriteType.Heart],
      scale: 1,
      method: (c) => {
        c.healthPoints++;
  ...
  public static IncreaseJumps(x: number, y: number, newMaxJmpCnt: number, name: string): Powerup {
    return new Powerup({
      ...
      y: y,
      name: name,
      instructions: `You can now jump ${newMaxJmpCnt} times.`,
      pauseDuration: 180,
      scale: 2,
      frames: PowerupFrames[PowerupSpriteType.KCoin],
      method: (c) => {
  ...
  public static Speed(x: number, y: number): Powerup {
    return new Powerup({
      ...
      y: y,
      name: 'Speed Boost',
      instructions: 'Press space to move fast!',
      pauseDuration: 180,
      scale: 1,
      frames: PowerupFrames[PowerupSpriteType.Run],
      method: (c) => {
        c.canRun = true;
      }
    });
  ...
  public static Complete(x: number, y: number): Powerup {
    return new Powerup({
      ...
      instructions: 'Congratulations! Compete against your friends for the fastest finish.',
      pauseDuration: 500,
      scale: 2,
      frames: PowerupFrames[PowerupSpriteType.KCoin],
      method: (c) => {}

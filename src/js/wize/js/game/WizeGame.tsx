...
export class WizeGame {
  ...
  tick() {
    ...
    this.character.setPosition(
      Math.max(Math.min(this.room.w - this.character.w, this.character.x), 0),
      this.room.hasCeiling ? Math.max(aboveBottomY, 0) : Math.min(this.character.y, aboveBottomY)
    );
  ...
  checkForCurrentPlatform() {
    ...
    this.room.damageZones.forEach((z) => {
      if (
        util.doRectanglesOverlap(
          this.character.x,
          this.character.y + this.character.h * 0.75,
          this.character.h * 0.25,
          this.character.w,
          z.x,
          z.y,
          20,
        ...
        this.character.setCurrentPlatform({
          x: z.x,

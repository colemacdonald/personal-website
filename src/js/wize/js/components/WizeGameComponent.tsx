...
export class WizeGameComponent extends Component {
  ...
  tick() {
    ...
    if (this.gameController.game.lastPowerup) {
      ...
      if (this.unPauseFrame === 0) {
        this.drawMessage(
          this.gameController.game.lastPowerup.name + ' acquired!',
          this.gameController.game.lastPowerup.instructions
        );
  ...
  render() {
    return (
      <div className="wize-game-component">
        <div className="wize-game-controls">
  ...
  drawBackground() {
    ...
    let backgroundImage = verticalRoom
      ? room.backgroundTheme.verticalBackgroundImage
      : room.backgroundTheme.horizontalBackgroundImage;
    ...
    if (backgroundImage) {
      ...
      this.cntx.fillRect(
        (room.w - this.viewportX) * this.canvasScale + margin,
        0,
  ...
  isInView(box: Rectangle): boolean {
    return util.doRectanglesOverlap(
      this.viewportX,
      this.viewportY,
      this.viewportH,
      this.viewportW,
  ...
  drawMinimap() {
    ...
    this.cntx.fillRect(
      minimap.x * this.canvasScale,
      minimap.y * this.canvasScale,
      minimap.w * this.canvasScale,
      minimap.h * this.canvasScale
    );
  ...
  drawMinimapDoors(scale, minimap) {
    ...
    this.drawOnMinimap(
      this.gameController.game.room.doors.map((d) => d.drawBox),
      scale,
      minimap
    );
  }
  ...
  drawMinimapCoins(scale, minimap) {
    ...
    this.drawOnMinimap(
      this.gameController.game.room.coins.map((c) => c.drawBox),
      scale,
      minimap
    );
    this.drawOnMinimap(
      this.gameController.game.room.powerups.map((p) => p.sprite.drawBox),
      scale,
      minimap
    );
  }
